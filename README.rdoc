...
You can filter the modules invoked by these javascript helpers with an optional final parameter:

    <script type="text/javascript">Analytical.track('/some/url', {filterModules: {google: true}});</script>
    <script type="text/javascript">Analytical.event('A javascript event', {with: 'data'}, {filterModules: {google: true}});</script>

To disable javascript helper functionality, use
